...
class Obat {
  ...
  //required -> wajib isi
  Obat(this.nama, {required this.dosisMg, required this.satuan});  //contoh named argument
...
class FormHapusObat {
  Future<void> hapusObat(String namaObat) async {
    print('Menghapus obat...');
    await Future.delayed(Duration(seconds: 1)); // simulasi proses input
    print('Obat "$namaObat" berhasil dihapus.');
  }
}
...
class MediTimeApp {
  ...
  Future<void> mulai() async {
    ...
    final j2 =
        await formTambahObat.buatJadwalBaru('Amoxicillin', 250, 'mg', WaktuMinum.Siang);
    final j3 =
        await formTambahObat.buatJadwalBaru('Vitamin C', 1000, 'mg', WaktuMinum.Sore);
...
Future<void> main() async {
  final profil = ProfilPengguna(
      ...
      preferensiNotifikasi: 'Getar'); //contoh position  argumen
